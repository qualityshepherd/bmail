...
export async function getEmailById (db, id) {
  return db
    .prepare('SELECT id, sender, sender_display, recipient, subject, body, message_id, in_reply_to, notify, starred, status, read, tags, cc, created_at FROM emails WHERE id = ?')
    .bind(id)
    .first()
}
